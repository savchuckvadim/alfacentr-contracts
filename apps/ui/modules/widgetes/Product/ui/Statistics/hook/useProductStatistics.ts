...
export const useProductStatistics = () => {
    ...
    const totalSum = items.reduce((sum, product) => sum + ((product.price || 0) * (product.quantity || 0)), 0)
    const ppkSum = ppkProducts.reduce((sum, product) => sum + ((product.price || 0) * (product.quantity || 0)), 0)
    const seminarSum = seminarProducts.reduce((sum, product) => sum + ((product.price || 0) * (product.quantity || 0)), 0)
    const upSum = upProducts.reduce((sum, product) => sum + ((product.price || 0) * (product.quantity || 0)), 0)

...
import { addParticipant, deleteParticipant, fetchParticipants, fetchProducts, getProductFieldByCodeValue, getProductSum, setFetchedProducts, setParticipants, setProducts, updateParticipant } from "@/modules/entities";
...
documentParagraphProductParticipantListener.startListening({
    ...
    effect: async (action, listenerApi) => {
        ...
        if (products.length > 0) {
            ...
            const searchedSeminarProducts = getProductsByType(products, 'seminar')
            if (searchedSeminarProducts && searchedSeminarProducts.length > 0) {
                const seminarNameVallue = getProductFieldByCodeValue(searchedSeminarProducts[0] as IAlfaProduct, 'SEMINAR_TOPIC')
                result += seminarNameVallue?.value + '\n'

                const seminarDateVallue = getProductFieldByCodeValue(searchedSeminarProducts[0] as IAlfaProduct, 'SEMINAR_START_AND_END_DATE')
                result += seminarDateVallue?.value + '\n'
                const seminarPlaceVallue = getProductFieldByCodeValue(searchedSeminarProducts[0] as IAlfaProduct, 'SEMINAR_PLACE')
                result += seminarPlaceVallue?.value + '\n'
                let participantsQuantity = 1

                if (participants && participants.length > 0) {
                    participantsQuantity = searchedSeminarProducts[0]?.quantity || 1
                }

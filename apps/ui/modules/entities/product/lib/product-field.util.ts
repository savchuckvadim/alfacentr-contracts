...
export const getProductFieldByCode = (product: IAlfaProduct, code: ProductFieldCode) => {
    return product.fields.find(field => field.bitrixId === bxProductData[code].bitrixId);
}

export const getProductFieldByCodeValue = (product: IAlfaProduct, code: ProductFieldCode) => {
...
export const getProductFieldValue = (field: IProductField) => {
    const rawValue = field.value as string | string[] | {
        value: string
        valueId: string
        valueEnum?: string
    }
...
export const filterProductFieldsForDetails = (fields: IProductField[]) => {
    ...
    const sortedFields = [...fields].sort(
        (a, b) => fieldsOrder.indexOf(a.bitrixId) - fieldsOrder.indexOf(b.bitrixId)
    );

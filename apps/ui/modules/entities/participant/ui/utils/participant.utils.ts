import { IParticipant, AlfaParticipantSmartItemUserFieldsEnum, BxParticipantsFieldNameEnum, BxParticipantsDataKeys, getParticipantSelectItemByValue } from "@alfa/entities";
...
export const getParticipantFormat = (participant: IParticipant): string => {
    ...
    let result = getParticipantSelectItemByValue(BxParticipantsDataKeys.format, value)?.label ?? 'Не выбрано'
...
export const getParticipantPrograms = (participant: IParticipant): {
    ...
    if (accountantGos) programs.push({ type: BxParticipantsFieldNameEnum.accountant_gos, value: accountantGos });
    if (accountantMedical) programs.push({ type: BxParticipantsFieldNameEnum.accountant_medical, value: accountantMedical });
    if (zakupki) programs.push({ type: BxParticipantsFieldNameEnum.zakupki, value: zakupki });
    if (kadry) programs.push({ type: BxParticipantsFieldNameEnum.kadry, value: kadry });
    if (corruption) programs.push({ type: BxParticipantsFieldNameEnum.corruption, value: corruption });
...
export const formatParticipantPrograms = (participant: IParticipant): string => {
    ...
    return programs.map(program => {
        ...
        if (program.type.includes('главных бухгалтеров и бухгалтеров бюджетной сферы')) {
            shortTypeName = 'Бухгалтеры бюджетной сферы'
        }
        if (program.type.includes('главных бухгалтеров и бухгалтеров государственного учреждения здравоохранения')) {
        ...
        if (program.type.includes('специалистов по антикоррупционной деятельности')) {

...
export const ParticipantStatistics = () => {
    const { participants, participantsCount, loading, error } =
        useParticipant();
    const cards = [
        {
        ...
        {
            title: 'Активные',
            value: participants.filter(p => p.stage === SmartStageEnum.CLIENT)
                .length,
            icon: 'check',
            color: 'green',
        },
        ...
        {
            title: 'Завершенные',
            value: participants.filter(p => p.stage === SmartStageEnum.SUCCESS)
                .length,
            icon: 'star',
            color: 'purple',
        },
    ] as IRaitingCardProps[];
    return <RatingCards cards={cards} />;
};

...
import {  RqItem, RQ_TYPE, BX_ADDRESS_TYPE, isFieldsEmpty, useBxRqEditAddress, AddressTypeId } from '@workspace/bx-rq'
...
export const BxRqAddressEdit = ({
  ...
  const handleSave = (typeId: BX_ADDRESS_TYPE) => {
    ...
    saveAddress(domain, companyId, clientType as RQ_TYPE, typeId, creating?.fields || [])
  }
  ...
  return (
    <div className="space-y-4">
      {addresses.map((address) => {
        ...
        const addressPercent = address.type_id === BX_ADDRESS_TYPE.REGISTERED ? registredPercent : primaryPercent
        return (
                ...
                <div className="flex items-center gap-2">
                  ...
                  {!isEmpty && (
                    <Badge variant="secondary"
                      className={
                        addressPercent > 75 ? 'bg-green-500' :
                          addressPercent > 20 ? 'bg-yellow-500' :
                            'bg-red-500'
                      }
                    >Заполнено {addressPercent}%</Badge>
                  )}
                  {isEmpty && (
                ...
                <div className="flex items-center gap-2">
                  {canCopy && (
                    <Button
                      ...
                      onClick={() => handleCopy(address.type_id)}
      ...
      {creating && creating.fields && creating.fields.length > 0 && (
        <BxRqEditModal
          ...
          onFieldChange={(code, value) => handleFieldChange(creating.type_id, code, value)}
        />
      )}
    </div>
  )

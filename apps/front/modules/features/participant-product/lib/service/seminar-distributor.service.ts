...
import {
    ...
    ParticipantSeminarFieldCodes,
} from '../../type/participant-ppk.type';
import {
    getParicipantPpkTopicsStats,
    getParicipantSeminarTopicsStats,
} from '../utils/participant-products';

type Topic = string;

export class SeminarDistributorService {
    ...
    constructor(participants: IParticipant[], products: IAlfaProduct[]) {
        ...
        this.products = getProductsByType(products, 'seminar') || [];
    ...
    public distribute(): IParticipantPpkMap {
        ...
        for (const [
            ...
            const products =
                this.topicProductMap.get(this.normalizeTopic(topic)) || [];
        ...
        this.participants.forEach(participant => {
            const participantPpkTopicsStats = getParicipantSeminarTopicsStats(
                participant,
                topicStats,
    ...
    private buildTopicToParticipantMap(): Map<Topic, IParticipant[]> {
        ...
        for (const participant of this.participants) {
            ...
            for (const topic of topics) {
                ...
                if (!map.has(normalizedTopic)) map.set(normalizedTopic, []);
                map.get(normalizedTopic)!.push(participant);
            }
        }
    ...
    private getParticipantTopics(participant: IParticipant): string[] {
        ...
        for (const field of participant.fields) {
            if (
                !ParticipantSeminarFieldCodes.includes(
    ...
    private normalizeTopic(str: string): string {
        return str
            .replace(/&quot;/g, '"') // HTML → обычные кавычки

...
export const useDocumentRq = () => {
    ...
    const { client, provider } = getForDocumentItems(rqs.client as DocumentRqAgent<RQ_TYPE.FIZ | RQ_TYPE.ORGANIZATION>, rqs.client?.type as RQ_TYPE.FIZ | RQ_TYPE.ORGANIZATION, rqs.provider as DocumentRqAgent<RQ_TYPE.ORGANIZATION>)
    return {
        client: client,

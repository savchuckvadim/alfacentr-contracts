...
export function SkeletonText({
  ...
  return <div className={cn("bg-gray-200 rounded animate-pulse", width, height, className)}></div>
}
...
export function PulseBar({ className }: { className?: string }) {
  return (
    <div className={cn("w-8 h-1 bg-gray-300 rounded-full overflow-hidden relative", className)}>
  ...
  return (
    <div className="max-w-2xl mx-auto p-8 space-y-8">
      <div className="text-center">
        ...
        <p className="text-gray-600">Elegant loading indicators for small components</p>
      </div>

      <div className="grid gap-6">
        {/* Inline text examples */}
        <div className="space-y-4">
          <h2 className="text-lg font-semibold">Inline Text Loading</h2>
          <div className="space-y-3 text-sm">
            <div className="flex items-center gap-2">
              <span>Status:</span>
              {loading ? <DotsLoader /> : <span className="text-green-600">Connected</span>}
            </div>
        ...
        {/* Card examples */}
        <div className="space-y-4">
          <h2 className="text-lg font-semibold">Card Content Loading</h2>
            ...
            <div className="space-y-2">
              {loading ? (
                ...
                <p className="text-sm text-gray-700">
                  This is some sample content that would appear after loading is complete. The skeleton loaders provide
                  a nice preview of the content structure.
                </p>
              )}
            </div>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            <button className="px-4 py-2 bg-blue-600 text-white rounded-md flex items-center gap-2 disabled:opacity-50">
              {loading && <TinySpinner className="border-white border-t-blue-200" />}
              {loading ? "Saving..." : "Save"}
            </button>

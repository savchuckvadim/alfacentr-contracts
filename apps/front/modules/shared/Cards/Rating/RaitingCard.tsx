...
export const RaitingCard = ({ title, value, color, icon }: IRaitingCardProps) => {
...
const getIcon = (icon: 'people' | 'check' | 'clock' | 'star', color: 'green' | 'blue' | 'red' | 'yellow' | 'violet' | 'orange' | 'purple') => {
    switch (icon) {
        case 'people':
        ...
        case 'star':
            return <Star size={17} color={color} className={` text-${color}-600`} />
    }
}
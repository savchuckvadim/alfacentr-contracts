...
export const ProductsProblems = () => {
    const { isLoading, topicStats, unassignedParticipants } =
        useParticipantPpk();
    ...
    if (totalUnassignedParticipants > 0) {
        infoItems.push(
            `${totalUnassignedParticipants} участников не назначены на ППК программы`,
        );
    }
    ...
    return (
        !isLoading &&
        (totalUnassignedParticipants > 0 || topicsWithDeficit > 0) && (
            <Info
                collapsible

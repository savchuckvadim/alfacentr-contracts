...
export interface ParticipantEditData {
    ...
    // Действия
    activateEditable: (participantId: number) => void;
    cancelEditable: () => void;
    changeEditable: (fieldCode: BxParticipantsDataKeys, value: string) => void;
    deleteParticipant: (participantId: number) => void;
    updateParticipant: () => void;
    editParticipantTopic: (fieldCode: BxParticipantsDataKeys, value: string) => void;
    
    // Форматтеры
    formatParticipantPrograms: (participant: any) => string;
}

export const useParticipantEdit = (participantId: number): ParticipantEditData => {

...
export class DocumentGenerateService {
    ...
    async generateDocument(dto: DocumentGenerateDto) {
        ...
        const file = await bitrix.file.downloadBitrixFileAndConvertToBase64(
            resultContractWithoutPt.downloadUrlMachine,
        );
        ...
        const updateDealDocumentsResponse = await bitrix.deal.update(entityId, {
    ...
    private async getInvoicesFiles(
        ...
        const invoiceDocWithoutPtFileData =
            await this.bitrix.file.downloadBitrixFileAndConvertToBase64(
                resultInvoiceWithoutPt.downloadUrlMachine,
            );

...
import { IRequestDocumentGenerateFieldsType, IRequestDocumentGenerateFieldValueType, IRequestDocumentGenerateType, RQ_TYPE } from "@alfa/entities";



class DocumentGenerateFieldValueDto implements IRequestDocumentGenerateFieldValueType {
    @ApiProperty({
...
export class DocumentGenerateDto implements IRequestDocumentGenerateType {
    ...
    @IsEnum(RQ_TYPE, {
        message: 'clientType должно быть одним из следующих значений: ' + Object.values(RQ_TYPE).join(', ')
    })
    clientType: RQ_TYPE
    @ApiProperty({
    ...
    @IsEnum(EContractType, {
        message: 'contractType должно быть одним из следующих значений: ' + Object.values(EContractType).join(', ')
    })

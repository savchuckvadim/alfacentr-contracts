...
type FListItem = { ID: string, SORT: string, VALUE: string, DEF: string, XML_ID: string }
const newField = {
    "ID": "10508",
...
export class AlfaCompanyController {
        ...
        while (condition) {
            ...
            if (resp && resp.result) {
                for (const key in resp.result) {
                    ...
                    await fs.appendFileSync('companies.json', JSON.stringify(element) + '\n');
                    lastId = element.ID;
                }

                console.log('lastId', lastId, results.length);
            }
    ...
    async updateCompanies() {
        ...
        const updateCompany = (companyId: number, regionId: number) => {
            const regionValue = oldList?.find(item => Number(item.ID) === regionId)?.VALUE;
            if (!regionValue) {
                console.log(`Region ${regionId} not found`);
                return;
            }
            const newRegionId = newField.LIST?.find(item => item.VALUE === regionValue)?.ID;
            if (newRegionId) {
        ...
        for (let i = 49; i < 50; i += 1) {
            ...
            for (const item of arr) {
                ...
                fs.appendFileSync(path, JSON.stringify({ ...item, file: i + 1 }) + '\n');

...
export class DealFieldHelperService {
     ...
     */
    public static updateDealDataFromBitrixResponse(
        bitrixResponse: AlfaBxField[],
    ): TDealData {
            ...
            if (key === BxDealDataKeys.participants) {
                for (const participant of Object.values(template[key])) {
                    for (const parKey in participant) {
                        if (
                            (participant[parKey] as TField).type ===
                            'enumeration'
                        ) {
                            const bitrixField = bitrixFields.find(
                                (f) =>
                                    f.bitrixId ===
                                    (participant[parKey] as TField).bitrixId,
                            );
                            if (bitrixField) {
                                (participant[parKey] as TFieldSelect).list =
                                    bitrixField.list || [];
                            }
                        }
                    }
                ...
                if ((template[key] as TField).type === 'enumeration') {
                    const bitrixField = bitrixFields.find(
                        (f) =>
                            f.bitrixId === (template[key] as TField).bitrixId,
                    );
                    if (bitrixField) {
                        (template[key] as TFieldSelect).list =
                            bitrixField.list || [];
                    }
                }
            }
     ...
     */
    private static flattenFieldsTemplate(
        template: TDealData,
    ): (TField | TFieldSelect)[] {
     ...
     */
    private static updateFieldProperties(
        field: DealField,
        bitrixField: AlfaBxField,
    ): void {
        if (bitrixField.name) {
    ...
    private static isEnumerationField(
        field: DealField,
    ): field is DealField & { type: 'enumeration' } {
     ...
     */
    private static updateEnumerationList(
        field: DealField & { type: 'enumeration' },
        list: NonNullable<AlfaBxField['list']>,
    ): void {

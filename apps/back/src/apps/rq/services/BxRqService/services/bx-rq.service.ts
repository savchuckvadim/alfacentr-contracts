...
export class BxRqService {
    ...
    private async processSingleRequisite(
        ...
        const addresses = this.addressService.processAddressesFromBatch(
            batches,
            requisite.ID,
        );
        const banks = this.bankService.processBanksFromBatch(
            batches,
            requisite.ID,
        );
        const customFields =
     ...
     * Обновляет адрес
     */
    async updateAddress(domain: string, address: any): Promise<boolean> {
        const { bitrix } = await this.pbxService.init(domain);
     ...
     * Обновляет банк
     */
    async updateBank(domain: string, bank: any): Promise<any> {
        const { bitrix } = await this.pbxService.init(domain);

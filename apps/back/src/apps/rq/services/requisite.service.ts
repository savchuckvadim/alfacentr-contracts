...
export class RequisiteService {
    ...
    async getRq(companyId: number, domain: string): Promise<BXRequisiteDTO[]> {
        ...
        const result = await bitrix.api.call<IBitrixResponse<BXRequisiteDTO[]>>(
            'crm.requisite.list',
            {
                filter: { ENTITY_ID: companyId },
            },
        );
        const resultRqs = result?.result as BXRequisiteDTO[];
        ...
        if (resultRqs?.length > 0) {
            ...
            for (const requisite of resultRqs) {
                ...
                const rqCommands: Array<{ key: string; value: any }> = [];
                ...
                for (const batch of batches) {
                    ...
                    for (const [resultKey, resultValue] of Object.entries(
                        ...
                        if (resultKey.includes('address')) {
                            const address = resultValue as any[];
                        ...
                        if (resultKey.includes('bankdetail')) {
                            const banks = resultValue as any[];
                        ...
                        for (const cmd of rqCommands) {
                            if (cmd.key === resultKey) {
                                ...
                                customField.EDIT_FORM_LABEL =
                                    ...
                                    customField.EDIT_FORM_LABEL;
                                customField.value = cmd.value;

                                const customFieldObj = new CustomField(
                                    customField,
                                );
                                if (
                                    !customFields.find(
    ...
    async createRq(
        ...
        const updateRqs: Record<string, any> = {};
    ...
    async updateAddress(domain: string, bxAddress: Address): Promise<boolean> {
        ...
        const typeIdValue =
            typeof bxAddress.TYPE_ID === 'number'
                ? bxAddress.TYPE_ID
                : (bxAddress.TYPE_ID as any)?.value || bxAddress.TYPE_ID;

        const updateAddress: Record<string, any> = {};
            ...
            for (const adr of addressCompany) {
                const adrTypeId =
                    typeof adr.TYPE_ID === 'number'
                        ? adr.TYPE_ID
                        : (adr.TYPE_ID as any)?.value || adr.TYPE_ID;

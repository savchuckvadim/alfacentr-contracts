...
export function DatePickerInput({
    ...
    // В контролируемом режиме дата мемоизируется, чтобы избежать нового Date на каждом рендере.
    const controlledDate = React.useMemo(() => {
        if (!controlledValue) return undefined;
        const parsed = new Date(String(controlledValue));
    ...
    return (
        <Field>
            ...
            <InputGroup className={hasError ? 'border-red-500' : undefined}>
                <InputGroupInput
                    id={inputId}
                    name={name}
                    value={currentValue}

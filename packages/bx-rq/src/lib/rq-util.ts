...
import { AddressRqItem, BankRqItem, EvsRqItem, ResolvedRQType } from "../type/evs-rq-type";
...
export const getRqFillPercent = (
  ...
  if (rq && rq.fields && rq.fields.length > 0) {
    ...
    const filledFields = filtredFields.filter(field => field.value !== null && field.value !== undefined && field.value !== '');
    ...
    if (rq.address && rq.address.items && rq.address.items.length > 0) {
      ...
      rq.address.items.forEach(item => {
        ...
        if (item.type_id === BX_ADDRESS_TYPE.PRIMARY || item.type_id === BX_ADDRESS_TYPE.REGISTERED) {
          fieldsCount += fields.length
          const filledFields = fields.filter(field => field.value !== null && field.value !== undefined && field.value !== '');
...
export const getBankFillPercent = (
  banks: BankRqItem[] | null,
): number => {
